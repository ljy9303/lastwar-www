...
import { buildQueryString } from "@/lib/api-service-hybrid"
...
export default function UsersPageClient() {
  ...
  // API 훅 사용
  const { execute: fetchUsers } = useApi<User[]>("/user")
  const { execute: deleteUserApi } = useApi("/user", "DELETE")
  ...
  // 유저 삭제 처리
  const handleDeleteUser = async (userSeq: number) => {
    try {
      await deleteUserApi(`/${userSeq}`)
      toast({
        title: "유저 삭제 성공",
          ...
          <Card>
            ...
            <CardContent>
              ...
              {isLoading ? (
                ...
                <UserList users={users} onEdit={handleEdit} onDeleted={handleDeleteUser} />
              )}
            </CardContent>
          </Card>
        </TabsContent>
